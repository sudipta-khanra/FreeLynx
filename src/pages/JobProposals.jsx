...
// 🛠️ নেকেড axios বদলে তোমার সেন্ট্রাল ইনস্ট্যান্সটি ইম্পোর্ট করা হলো
...
export default function JobProposals() {
  ...
  const fetchProposals = async () => {
    try {
      ...
      // 🛠️ ইউআরএল ক্লিন করা হলো এবং হেডার্স বাদ দেওয়া হলো (ইনস্ট্যান্স অটোমেটিক টোকেন হ্যান্ডেল করবে)
      const res = await axios.get(`/proposals/${actualJobId}`);
      console.log('Proposals API response:', res.data);

      setProposals(res.data);

      // 🛠️ জব টাইটেল ফেচ করার ইউআরএল-ও ক্লিন করা হলো
      const jobRes = await axios.get(`/jobs/${actualJobId}`);
          ...
          {proposals.map((p) => (
            <div key={p._id} className="border p-4 rounded shadow">
              <h2 className="text-lg font-semibold text-gray-800">
                {p.freelancerId?.name || 'Unknown Freelancer'} (
                {p.freelancerId?.email || 'No Email'})
              </h2>
