...
function Pagination({ totalPages, currentPage, onPageChange }) {
  const maxButtons = 7; // max visible page numbers

  // Generate page numbers with dots for long pagination
  const getPageNumbers = () => {
    const pages = [];
    const siblingCount = 1; // pages to show on each side of currentPage
    const totalPageNumbers = siblingCount * 2 + 2; // 5 = first, last, current, 2 dots

    if (totalPages <= totalPageNumbers) {
      // Show all pages if total pages less than the limit
